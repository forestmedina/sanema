...
generate_local_variable_access(sanema::ByteCode&byte_code, sanema::ByteCodeCompiler::Scope&context_frame_aux,
                               ...
                               sanema::ByteCodeCompiler::GeneratorsMap&generator_map) {
  //The commented section could be used later for when the function need to access the parameters
...
generate_function_call(
  ...
  for (auto&argument: function_call.arguments) {
    ...
    function_definition.parameter.emplace_back("",
                                               sanema::FunctionParameter::Modifier::VALUE,
                                               type);
  }
  auto final_function_definition = context_frame_aux.function_collection.find_function(function_definition);
  if (!final_function_definition.has_value()) {
    std::string message = std::format("function {}  (",
                                      function_call.identifier);
    std::string separator = "";
    for (auto&parameters: function_definition.parameter) {
      message += std::format("{} {}",
                             separator,
