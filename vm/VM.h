...
union OperandType{
 ...
 explicit  operator std::int8_t() const { return sint64_v; }

  // Conversion operator for int16_t
  explicit operator std::int16_t() const { return sint64_v; }

  // Conversion operator for int32_t
 explicit  operator std::int32_t() const { return sint64_v; }
...
private:
  ...
  inline void set_local(IPType& ip){
      ...
      context_frame.write<type>(address, value);
  }
